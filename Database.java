...
public class Database {
	...
	//method for getting the user account
	...
	//method for adding the new account
	void addNewAccount(Account ac) {
		this.account.add(ac);
	} //end addNewAccount
	
	//boolean, check if the user account number unique or not
	boolean isAccountNumberUnique(String n) {
		...
		return true;
	} //end isAccountNumberUnique
	
	//method for saving the user account informaton so they can reuse the program
	void saveData() {
		System.out.println("\nYour data is saved");
		try {
			...
			bw.close(); //closing buffer to end operation
		} 
		catch (IOException e) {
			e.printStackTrace();
		}		
	...
	//method for printing the accounts in the program
	void printAccounts() {
		for(int i=0; i<account.size(); i++) {
	...
	//method for loading the user data
	void loadData() {
		try {
			...
			br.close(); //close the buffer to end operation
		} 
		catch (IOException e) {
			e.printStackTrace();
		}
